...
# target=2
# targets = np.full((train_labels.shape[0],), target)
# train_labels = np.eye(train_labels.shape[-1])[targets]
...
target=2
targets = np.full((test_labels.shape[0],), target)
test_labels = np.eye(test_labels.shape[-1])[targets]
print(test_labels)
...
def adv_loss(preds, labels, is_targeted):
  ...
  labels = np.eye(labels.shape[-1])[targets]
...
def generator_loss(disc_generated_output, gen_output, target,pred,label):
  ...
  tf.print("L ADV  - ",l_adv)
  tf.print("gan_loss   ", gan_loss)
  tf.print("l1 loss   ",l1_loss)
...
def generate_images(model,test_input,t,a,b):
  ...
  for i in range(2):
    ...
    plt.axis('off')
  if(a=='train'):
    plt.savefig('target_output_images/train_'+str(b)+'.png')
  elif(a=='test'):
    plt.savefig('target_output_images/test_'+str(b)+'.png')
