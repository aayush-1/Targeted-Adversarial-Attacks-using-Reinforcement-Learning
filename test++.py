...
# print(test_dataset_input_gen)
# tf.reshape(test_dataset_input_gen,[test_dataset_input_gen.get_shape[0],28,28,1])
acc1=0
acc2=0
# prediction = generator(test_dataset_input_gen.take(10000), training=True)
# (loss, accuracy)=target_model.evaluate(generator(prediction,test_labels , verbose = 1))
# print("accuracy = ",accuracy)
for test_input,t in test_dataset.take(10000):
  test_input=tf.reshape(test_input,[1,7,7,50])
  prediction = generator(test_input, training=True)
  pred_fake=np.argmax(target_model.predict(prediction))
  pred_orig=np.argmax(target_model.predict(test_dataset1[X].reshape(1,28,28,1)))
  t=np.argmax(t)
  if( pred_fake!=t):
    acc2+=1
    if(pred_orig==t):
